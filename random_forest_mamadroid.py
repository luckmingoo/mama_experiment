...
def get_test_data(test_id, method, save_feature_dict):
    root_dir = '/home/mlsnrs/apks/ssd_1T/mamadroid/light_weight_dataset/%s' % method
    test_data_md5 = []
    test_data_dir = '/home/mlsnrs/apks/VirusShare/dataset_s_baseline/dataset_20132014_light_weight'
...
def get_train_data(method, save_feature_dict):
    root_dir = '/home/mlsnrs/apks/ssd_1T/mamadroid/light_weight_dataset/%s' % method
    train_data_md5 = []
    train_data_dir = '/home/mlsnrs/apks/VirusShare/dataset_s_baseline/dataset_20132014_light_weight'
...
def evaluation(method): 
    log_name = 'log/%s_evaluation.txt' % method
    if os.path.exists(log_name):
        os.remove(log_name)
    save_feature_path = '/home/mlsnrs/apks/ssd_1T/mamadroid/light_weight_dataset/%s/%s_save_feature_list.csv' % (method, method)
    save_feature_dict = get_save_feature_dict(save_feature_path)
    print('have read save_feature_dict: %d' % len(save_feature_dict))
    x_train, y_train = get_train_data(method, save_feature_dict)
    ...
    for test_id in range(0, 13):
        x_test, y_test = get_test_data(test_id, method, save_feature_dict)
