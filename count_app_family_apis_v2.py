...
def parse_family_app_periods_droidevolver_sensitive_method(family_app_periods, feature_path_dict, family_name):
...
#                 f.write('\n')
    ...
    with open('save_diff_period_pickle/%s_diff_period_sensitive.pkl' % family_name, 'wb') as f:
        pkl.dump(diff_period, f)
    with open('save_diff_period_pickle/%s_diff_p0_period_sensitive.pkl' % family_name, 'wb') as f:
        pkl.dump(diff_p0_period, f)
    with open('save_diff_period_pickle/%s_diff_all_period_sensitive.pkl' % family_name, 'wb') as f:
...
#         periods_api.append(period_api_set)
#         if idx != 0:
#             common_apis_diff_freq_dict = diff_two_period_common_apis(previous_period_api_dict, previous_cnt, next_period_api_dict, next_cnt)
#             
#             with open('save_diff_type_apis/%s_p%d_common_api_list.txt' % (family_name, idx), 'w') as f:
#                 f.write('\n'.join(common_previous_api_set))
#                 f.write('\n')
...
def parse_family_app_periods_droidevolver_common_apis(family_app_periods, feature_path_dict, family_name):
    periods_api = []
    root_dir = '/mnt/AndroZoo/DroidEvolver_feature'
    idx = 0
    diff_period = []
    ...
    for period in family_app_periods: # period ['%d-%d' % (period_start_year_month, period_end_year_month), period_row]
        ...
        periods_api.append(period_api_dict)
#         if idx != 0:
#             diff_two_period_common_apis(periods_api[idx - 1], previous_cnt, next_period_api_dict, next_cnt)
#             
#         idx += 1
# 
#     with open('save_diff_period_pickle/%s_diff_period.pkl' % family_name, 'wb') as f:
...
def parse_two_periods_commmon_apis_sensitive(previous_period, next_period, feature_path_dict, family_name, diff_frequency_bound_list):
    sensitive_apis = get_sensitive_api()
    sensitive_methods = set()
    for method in sensitive_apis: # translate the format of method string 
        method = method.replace('->', ':')
    ...
    with open('save_diff_period_pickle/%s_common_apis_diff_freq_sensitive.pkl' % family_name, 'wb') as f:
        pkl.dump(common_apis_diff_freq_dict, f)
...
def plot_two_period_with_diff_frequency():
    families_diff = []
    with open('proportional_division_top_families_diff_droidevolver_with_frequency_0.10.csv') as f:
        reader = csv.reader(f) 
        for row in reader:
            family_name = row[0]
    ...
    plt.savefig('diff_api/proportional_division_malware_families_period_diff_apis_with_diff_freq.png', dpi = 300)
...
#     plt.bar(families_x_label, families_add_label, color = '#4169E1', label = 'family add apis', bottom = families_common_label + families_diff_freq_label)
    ...
    plt.savefig('diff_api/proportional_division_malware_families_period_diff_apis_with_diff_freq_simplified.png', dpi = 300)
...
def plot_all_period_average():
    families_diff = []
    with open('./diff_api/proportional_division_all_periods_api_changs.csv') as f:
        reader = csv.reader(f) 
        for row in reader:
            family_name = row[0]
            previous_period_app_num = int(row[1])
...
def plot_two_period():
    families_diff = []
    with open('proportional_division_top_families_diff_droidevolver_0.10.csv') as f:
        reader = csv.reader(f) 
        for row in reader:
            family_name = row[0]
            previous_period_app_num = int(row[1])
...
def plot_all_period_average_raw_data_sensitive(families_average_diff): # family_name, per_period_app_num, delete_average_api_num, common_average_api_num, add_average_api_num
    families_x_label = []
    families_delete_label = []
    families_common_label = []
    families_add_label = []
    ...
    plt.cla()
    plt.figure(figsize = (10, 8))
    plt.bar(families_x_label, - families_delete_label, color = '#00BFFF', label = 'delete sensitive apis')
    plt.bar(families_x_label, families_common_label, color = '#B0C4DE', label = 'common sensitive apis')
    plt.bar(families_x_label, families_add_label, color = '#4169E1', label = 'add sensitive apis', bottom = families_common_label)
    plt.tick_params(labelsize=8)
    plt.xticks(rotation = 55)
    plt.legend()
#     plt.title('malware families period diff apis')
#     plt.savefig('diff_api/proportional_division_malware_families_period_diff_apis.png', dpi = 300)
    ...
    foo_fig = plt.gcf()
    foo_fig.savefig('diff_api/average_diff_apis_between_periods_sensitive.eps', format='eps', dpi = 300) 
    plt.savefig('diff_api/average_diff_apis_between_periods_sensitive.jpg', dpi = 300)  

def plot_two_period_sensitive_method():
    families_diff = []
    with open('proportional_division_top_families_diff_droidevolver_sensitive_method_0.10.csv') as f:
        reader = csv.reader(f) 
        for row in reader:
            family_name = row[0]
            previous_period_app_num = int(row[1])
    ...
    plt.legend()
#     plt.title('malware families period diff sensitive apis')
    plt.savefig('diff_sensitive_api/proportional_division_malware_families_period_diff_sensitive_apis.png', dpi = 300)
 
    foo_fig = plt.gcf()
    foo_fig.savefig('diff_api/diff_sensitive_apis_between_periods.eps', format='eps', dpi = 300) 
    plt.savefig('diff_api/diff_sensitive_apis_between_periods.jpg', dpi = 300)      
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        families_diff.append(family_diff)
    with open('proportional_division_top_families_diff_droidevolver_%0.2f.csv' % min_rate, 'wb') as f:
        writer = csv.writer(f)
        writer.writerows(families_diff)
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        families_diff.append(family_diff)
    with open('proportional_division_top_families_diff_droidevolver_sensitive_method_%0.2f.csv' % min_rate, 'wb') as f:
        writer = csv.writer(f)
        writer.writerows(families_diff)
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        families_diff.append(family_diff)
    with open('proportional_division_top_families_diff_droidevolver_with_frequency_%0.2f.csv' % min_rate, 'wb') as f:
        writer = csv.writer(f)
        writer.writerows(families_diff)
...
# #         next_end_year_month = update_stop_time(period_end_year_month, 3)         
        ...
        families_diff.append(family_diff)
    with open('proportional_division_top_families_diff_droidevolver_with_frequency_sensitive_method_%0.2f.csv' % min_rate, 'wb') as f:
        writer = csv.writer(f)
        writer.writerows(families_diff)
...
def count_sensitive_api_evolver_with_periods_in_family(min_x_month_after, min_rate):
    feature_path_dict = get_droidevolver_feature_path_dict()
    malware_dataset_path = 'dataset_euphony_family_filted.csv' # [md5, family, support_num, first_seen, vt_cnt]
    family_app = {} # key = family_name, value = [[md5, first_year_month]
    with open(malware_dataset_path, 'r') as f:
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        for row in family_app_periods:
            print('%s:%d ' % (row[0], len(row[1]))),
        print('')
        parse_family_app_periods_droidevolver_sensitive_method(family_app_periods, feature_path_dict, family_name)

def count_api_evolver_with_periods_in_family_common_apis(min_x_month_after, min_rate):
    feature_path_dict = get_droidevolver_feature_path_dict()
    malware_dataset_path = 'dataset_euphony_family_filted.csv' # [md5, family, support_num, first_seen, vt_cnt]
    family_app = {} # key = family_name, value = [[md5, first_year_month]
    with open(malware_dataset_path, 'r') as f:
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        for row in family_app_periods:
            print('%s:%d ' % (row[0], len(row[1]))),
        print('')
        parse_family_app_periods_droidevolver_common_apis(family_app_periods, feature_path_dict, family_name)
...
def count_api_unstability_v5_sensitive(): # get api from droidevolver feature
    diff_frequency_bound_list = [0.1, 0.3, 0.5, 1.0]
    feature_path_dict = get_droidevolver_feature_path_dict()
    malware_dataset_path = 'dataset_euphony_family_filted.csv' # [md5, family, support_num, first_seen, vt_cnt]
    family_app = {} # key = family_name, value = [[md5, first_year_month]
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        previous_period = ['%s -> %s' % (previous_period_row[0][1], previous_period_row[-1][1]), previous_period_row]
        next_period = ['%s -> %s' % (next_period_row[0][1], next_period_row[-1][1]), next_period_row]
        family_diff = parse_two_periods_commmon_apis_sensitive(previous_period, next_period, feature_path_dict, family_name, diff_frequency_bound_list)
        families_diff.append(family_diff)
    with open('proportional_division_top_families_diff_common_apis_sensitive.csv', 'wb') as f:
        writer = csv.writer(f)
        writer.writerows(families_diff)
...
def count_all_perios_api_changes_average():
    feature_path_dict = get_droidevolver_feature_path_dict()
    malware_dataset_path = 'dataset_euphony_family_filted.csv' # [md5, family, support_num, first_seen, vt_cnt]
    family_app = {} # key = family_name, value = [[md5, first_year_month]
    with open(malware_dataset_path, 'r') as f:
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        delete_previous_api_num_sum = 0
        common_previous_api_num_sum = 0
        add_previous_api_num_sum = 0
        idx = 0
        for row in diff_all_period:
            idx += 1
            if idx <= 1:
...
def count_all_perios_sensitive_api_changes_average():
    feature_path_dict = get_droidevolver_feature_path_dict()
    malware_dataset_path = 'dataset_euphony_family_filted.csv' # [md5, family, support_num, first_seen, vt_cnt]
    family_app = {} # key = family_name, value = [[md5, first_year_month]
    with open(malware_dataset_path, 'r') as f:
...
#             first_year_month = int(first_seen.split('-')[0] + first_seen.split('-')[1])
    ...
    for family_name in family_app: # 'airpush', 'smsreg', 'fakeinst', 'gappusin', 'youmi', 'dowgin', 'adwo', 'kuguo', 'secapk', 'droidkungfu'
        ...
        all_periods_api_changes.append([family_name, len(family_app[family_name])])
        with open('save_diff_period_pickle/%s_diff_all_period_sensitive.pkl' % family_name, 'rb') as f:
            diff_all_period = pkl.load(f) # [period_time, len(delete_previous_api_set), len(common_previous_api_set), len(add_previous_api_set)]
        print(len(diff_all_period))
        ...
        add_average_api_num = add_previous_api_num_sum/9
        families_average_diff.append([family_name, per_period_app_num, delete_average_api_num, common_average_api_num, add_average_api_num])
    plot_all_period_average_raw_data_sensitive(families_average_diff)
    print('finish')
...
def plot_families_common_apis_diff_freq_sensitive():
    #[family_name, previous_cnt, previous_period_time, next_cnt, next_period_time]
    diff_frequency_bound_list = [0.1, 0.3, 0.5, 1.0]
    families_common_apis_diff = []
    new_families_common_apis_diff = []
    with open('proportional_division_top_families_diff_common_apis_sensitive.csv', 'rb') as f:
        reader = csv.reader(f) 
        for row in reader:
            family_name = row[0]
            family_common_apis_diff = [family_name]
            ...
            families_common_apis_diff.append(family_common_apis_diff)
            new_families_common_apis_diff.append(new_family_common_apis_diff)
    with open('api_frequency_common_apis_sensitive.csv', 'wb') as f:
        writer = csv.writer(f) 
        writer.writerows(new_families_common_apis_diff)
...
#     colors = ['#D8BFD8', '#DDA0DD', '#EE82EE', '#FF00FF', '#EE82EE']
    ...
    plt.legend()
#     plt.title('malware families period diff apis')
    plt.savefig('diff_api/proportional_division_malware_families_periods_common_apis_sensitive.png', dpi = 300)
    foo_fig = plt.gcf()
    foo_fig.savefig('diff_api/malware_families_common_apis_evolver_sensitive.eps', format='eps', dpi = 300) 
    plt.savefig('diff_api/malware_families_common_apis_evolver_sensitive.jpg', dpi = 300)  
    print('finish')    
...
#     count_all_perios_api_changes_average()
    ...
    # count sensitive method
#     count_api_unstability_v3_sensitive_method(3, 0.1)
#     count_sensitive_api_evolver_with_periods_in_family(3, 0.1)
#     plot_two_period_sensitive_method()
#     count_all_perios_sensitive_api_changes_average()
    count_api_unstability_v5_sensitive()
    plot_families_common_apis_diff_freq_sensitive()
